    ...
    public function run(){
        ...
        while(true){
            ...
            if($this->status === 501 and $lastCheck <= (time() - 5)){
                ...
                if(isset($np["hash"]) and $np["id"] !== $lastPlaying){
                    $lastPlaying = $np["id"];
                    $l = sendKawaApiMessage("/listeners");
                    sendIRCMessage("Now playing for ".count($l)." listener(s): " . formatTrackName($np), BOT_NP_CHANNEL);
                }
            }
        }
    }
